...
Mat get_normed_affine_transform(const vector<Point2f> & src_tri ,bool obj_to_oc = true)
{
	...
	vector<Point2f> dst_trip; //refer to obj_tripple
	double edge = 0;
...
vector<Point2f> get_centre(const Mat & img_src, const vector<Rect2f> & roi_trip)
{
	vector<Point2f> trip_centre;
	vector<Mat> roi_img;
	for (auto & r : roi_trip)
	{
		roi_img.push_back(img_src(r));
	}
	for (size_t i = 0 ; i < roi_img.size() ; ++i)
	{
		Mat img = roi_img[i];
		GaussianBlur(img, img, Size(3, 3), 0);
		threshold(img, img, 0, 255, CV_THRESH_OTSU);
		Mat element = getStructuringElement(MORPH_RECT, Size(3, 3));
		morphologyEx(img, img, MORPH_OPEN, element, Point(-1, -1), 1 , BORDER_ISOLATED, Scalar(0));
		morphologyEx(img, img, MORPH_CLOSE, element, Point(-1, -1), 1 , BORDER_ISOLATED, Scalar(0));

		vector<vector<Point>> contours;
		map<size_t, vector<Point> , greater<size_t> > area_map;
		cv::findContours(img, contours, RETR_EXTERNAL, CHAIN_APPROX_SIMPLE, roi_trip[i].tl());
		
		for (auto & c : contours)
		{
			area_map[moments(Mat(c)).m00] = c;
		}
		vector<Point> obj = area_map.begin()->second;
		Moments Mpq = moments(obj, true);	
		trip_centre.push_back( Point2f(static_cast<float>(Mpq.m10 / Mpq.m00), static_cast<float>(Mpq.m01 / Mpq.m00)) );
	}
	return trip_centre;
}
...
vector<Point2f> matching_template_accelerated_knn(const Mat & _img, const Mat & _mark, double scaler_f = 0.3, double similarity = 0.95, int k = 3, double roi_scaler = 0.2)
{
	...
	for (int i = 0; i < k; ++i)
	{
		...
		Rect2f roi_ = Rect2f(roi.x / scaler_f, roi.y / scaler_f, roi.width / scaler_f - 1, roi.height / scaler_f - 1);
		roi_trip.push_back(roi_);
	}
	Mat img_s = _img.clone();
	vector<Point2f> src_trip = get_centre(img_s, roi_trip);
...
public:
	...
	diff_map(size_t _n = 0 , Mat _diff = Mat() , double _iou  = 0, double _max_s = 0)
	{
		n = _n;
		diff = _diff;
		iou = _iou;
	...
	//bool operator < (diff_map const & res ) const
	//{
	//	return this->n < res.n;
	//}
};
...
vector<Rect2d> matching_template_rois(Mat & _img, Mat & _patch , double scaler_f = 0.2 , double similarity = 0.8, int k = 32, double roi_scaler = 0.2 ,double roi_expend = 0.05 )
{
	Mat img, patch;
	...
	//cv::resize(_img, img, Size(), scaler_f2, scaler_f2, INTER_AREA);
	...
	for (size_t i = 0; i < k; ++i)
	{
		...
		roi = Rect2d((roi.x / scaler_f), (roi.y / scaler_f), (roi.width) / scaler_f , (roi.height) / scaler_f);

		x1 = roi.x - roi.width * roi_expend > 0 ? roi.x - roi.width * roi_expend : 0;
		y1 = roi.y - roi.height* roi_expend > 0 ? roi.y - roi.height* roi_expend : 0;
		x2 = roi.x + roi.width * (1+ roi_expend) > _img.cols-1 ? _img.cols - 1 : roi.x + roi.width * (1 + roi_expend);
		y2 = roi.y + roi.height* (1+ roi_expend) > _img.rows-1 ? _img.rows - 1 : roi.y + roi.height* (1 + roi_expend);
		Rect2f roi_exp = Rect2f(Point2f(x1, y1), Point2f(x2, y2));

		//Rect2d _roi = Rect2d((roi_exp.x / scaler_f), (roi_exp.y / scaler_f), ( (roi_exp.width)/ scaler_f) - 1, ((roi_exp.height) / scaler_f) - 1);
		//rectangle(_img, _roi, Scalar(222), 1, LINE_4);
		//if (_roi.x + _roi.width > _img.cols) _roi.width -= (_roi.x + _roi.width - _img.cols+1);
		//if (_roi.y + _roi.height > _img.rows) _roi.height -= (_roi.x + _roi.height - _img.rows+1);

		obj_st.insert(pair<float, Rect2d>(roi_exp.tl().x, roi_exp));
	}
...
Obj_Set get_std_temp(Mat & _std_temp , double area_th = 0.00005)
{
	Obj_Set std_temp(0, _std_temp.clone());
	Mat & obj_img = std_temp.obj_img;
	...
	std_temp.centre = cen;
	std_temp.master_dir = master_dir;
	std_temp.master_vec = master_vec;
	std_temp.ang_diff = 0;

	return std_temp;
}

void matching_template_accelerateds( vector<Mat> & objs ,Mat & std_temp ,vector<diff_map> & diff_set , double area_th = 0.01)
{
		int i = 0;
...
vector<Point2f> match_knn_img_patch_centre( Mat & _img,  Mat & _patch, int k = 3 , double similarity = 0.95 , double roi_scaler = 0.1 , double scaler_f = 0.1)
{
	...
	vector<Point2f> src_trip = get_centre(_img, roi_trip);
	right_hand_based_tripple_sort(src_trip);
	return src_trip;
}
...
VideoCapture gen_video_from_img(string img_path, int frame_num = 1200,int frame_rate = 30, double speed = 0.01 , double black_edge_expend = 2)
{
...
public:
	...
	ml_obj( double _area_th = 0.00005) : area_th(_area_th) {};
...
void locate_tripple_mark(const size_t & n)
	...
	{	
		...
		//for (auto & obj : objs)
		//{
		//	obj_st.insert(pair<float, Obj_Set>(i, Obj_Set((size_t)i, obj.clone())));
		//	++i;
		//}
		for (auto & _obj : obj_st)
		{
		...
		for (auto & os : obj_st)
		{
...
#endif
			absdiff(std_temp, obj_img, diff);
			double iou = sum(diff)[0] / sum(std_temp)[0];
			multimap<double, Rect2f, greater<double>> indesity;
			size_t s = min(std.obj_img.cols , std.obj_img.rows) / 16 + 3;
			Size S(s, s);			
			/*
			Mat std_temp_clone_temp = std_temp.clone();
			if (std_temp_clone_temp.type() != CV_8UC1)std_temp_clone_temp.convertTo(std_temp_clone_temp, CV_8UC1);
			for (int iter = 0; iter < 8; iter++) {
				int size_borden = sqrt(S.width * S.width + S.height * S.height);

				Rect2i rect_bound = boundingRect(std_temp_clone_temp);
				int borden_x = std_temp_clone_temp.cols - (rect_bound.x + rect_bound.width + 4);
				if (borden_x < size_borden)
					S.width = borden_x;
				int borden_y = std_temp_clone_temp.rows - (rect_bound.y + rect_bound.height + 4);
				if (borden_y < size_borden)
					S.height = borden_y;

				int rect_size = min(S.width, S.height);
				S = Size(rect_size, rect_size);
				int max_rect_size = min(std_temp_clone_temp.rows, std_temp_clone_temp.cols) / 8;
				if (rect_size < 0 || rect_size>max_rect_size) {
					s /= 2;
					S = Size(s, s);
				}
				else break;
			...
			*/
			...
			int stride = min(S.height, S.width) / 4;
			matrix_partitions(std_temp, indesity, S, stride , diff);
			
			double max_width = CalculateWidthDiff(obj_img, std_temp, indesity);

			diff_mxt.lock();
			ml_obj::diff_set[n] = diff_map(n, diff, iou, max_width);
			ml_obj::objs.erase(n);
			diff_mxt.unlock();
		}
		return;
...
map<size_t, vector<Point2f>> ml_obj::Ow_objs = map<size_t, vector<Point2f>>();
	...
	//			thread ts(&ml_obj::matching_template_accelerateds, &obj, i);
	//			ts.detach();
	//			//ts.join();
	...
	while (1)
	{
		...
		time = (double)getTickCount();
		matching_template_accelerateds(img_t, temp_t , diff_set);
...
void calibra2(vector<Mat> &vec_mat,Pattern type=CHESSBOARD,Size board_num=Size(11,8),Size2f real_size=Size2f(3.0,3.0))
{
	...
	{
		int ch = 0;
	    cout << "Do you need to recalibrate? " << endl;
		cout << "if not press Enter,will automatically use original xml after 3s" << endl;
...
void rechoose_tmp(Mat& temp, string img_path,string temp_path, double scaler = 1.0) {
	int waitime = 5;
	cout << "Do you want to re-select template image,yes or no? y/n ."
		"\nIf not press anykey,will automatically continue after "<<waitime<<"s" << endl;
	char ch ;
	std::chrono::high_resolution_clock::time_point startTime = std::chrono::high_resolution_clock::now();
	std::chrono::duration<double> time_span = std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now()-startTime);
	for(;time_span.count()<waitime;)
	{
		if (_kbhit()) //如果有按键按下，则_kbhit()函数返回真
		{
			...
			if (ch == 'y')
			{
				cout << "Please press the SPACE key to select the template in the display window." << endl;
...
void obj_scale_detect(string img_path , string temp_path , double scaler = 1.0 , vector<Mat> vec_mat = vector<Mat>(), bool diff_calc_en = true , bool laser_mark_en = true , string mark_path = string() , vector<Point2f> Obj_point_in_Oc = vector<Point2f>())
{
	Mat temp = imread(temp_path, IMREAD_GRAYSCALE);
	rechoose_tmp(temp, img_path, temp_path);
	if (temp.empty())return;
	cv::resize(temp, temp, Size(), scaler, scaler, INTER_AREA);
	img_pre_processing(temp);
	Mat _mark;

	if (diff_calc_en)
	{
		ml_obj::std = get_std_temp(temp);
	}
	if (laser_mark_en)
	{
		vector<Point2f> obj_points;
		_mark = imread(mark_path, IMREAD_GRAYSCALE);
		if (!_mark.empty())
		{
			resize(_mark, _mark, Size(), scaler, scaler, INTER_AREA);
			img_pre_processing(_mark);
			ml_obj::mark = _mark;
			ml_obj::vec_mat_H = vec_mat;

			if (Obj_point_in_Oc.empty()) Obj_point_in_Oc = matching_template_accelerated_knn(temp, _mark);
			if (!Obj_point_in_Oc.empty())
			{
				vector<Point2f> src_tri;
				src_tri = matching_template_accelerated_knn(temp, _mark);
				if (src_tri.size() == 3)
				{
					Mat A = get_normed_affine_transform(src_tri, false);
					if (!A.empty())
		...
		double d_frame = 0;
		double frames_per_obj = 1;
		double obj_num = 1;
		double wait_frames = frames_per_obj * obj_num;
		...
		const int frame_error_times = 5;
		int error_times = frame_error_times;

		for (Mat & frame = get_next_frame(img_path, scaler); error_times; frame = get_next_frame())
		{
			if (frame.empty())
			{
				--error_times;
			...
			error_times = frame_error_times;

#ifdef RUN_TIME_CALC
			frame_num++;
			double tpf = (((double)getTickCount() - time3) / getTickFrequency()) * 1000;
			cout << "time per frame: " << tpf << " ms. " << frame_num << endl;
			...
			++d_frame;
			cv::resize(frame, tf, Size(), tracking_scaler, tracking_scaler);		
			vector<Rect2d> new_rois;
			if ( --wait_frames <= 0 )
			{
				img_pre_processing(frame);
				vector<Rect2d> rois = matching_template_rois(frame, temp);

				const double iou_th = 0.2;
				for (auto & roi : rois)
				{
					double max_iou = 0;
					Rect2d r = Rect2d(roi.tl() * tracking_scaler, roi.br() * tracking_scaler);
					multiset<double, greater<double>> max_roi;
					for (auto & t_box : tracking_box)
					{
			...
			if (!new_rois.empty())
			{
				if (mts_master.try_lock())
				{
					...
					time = (double)getTickCount();
#endif				
					frames_per_obj = d_frame / new_rois.size();
					wait_frames = frames_per_obj * obj_num;

					d_frame = 0;
					ml_obj::objs.clear();
					ml_obj::objs_mark.clear();
					...
					for (int i = 0; i < new_rois.size(); ++i, ++n)
					{
						...
						if (!_mark.empty() && laser_mark_en)
						{
							ml_obj::objs_mark[n] = frame(new_rois[i]);
							ml_obj::Oxy[n] = new_rois[i].tl();
							thread ts_mark(&ml_obj::locate_tripple_mark, &obj, n);
							ts_mark.detach();
							//ts_mark.join();
						}
						if (diff_calc_en)
						{
							ml_obj::objs[n] = frame(new_rois[i]);
							ml_obj::lens_per_pixel = get_scale_ds(vec_mat);
...
#ifdef RUN_TIME_CALC
					cout << "time new tracker: " << (((double)getTickCount() - time4) / getTickFrequency()) * 1000 << endl;
#endif
				}
				else
				{
			...
			for (auto it = tracking_set.begin(); it != tracking_set.end(); )
			{
				...
				if(tracking_box[i].br().x > tf.cols-1|| tracking_box[i].br().y > tf.rows - 1|| tracking_box[i].tl().x < 0|| tracking_box[i].tl().y < 0)
				{
					tracking_set.erase(it++);
					tracking_box.erase(i);
					continue;
			...
			//if ( ml_obj::objs.empty() && ml_obj::objs_mark.empty() && (!calc_end[0]) )
			//{					
			//	cout << "calc over !" << endl;
			//	calc_end[0] = true;
				//waitKey(1000);
			...
			bool mark_over = !ml_obj::Ow_objs.empty() ? ml_obj::objs_mark.empty() : true ;
			bool diff_over = diff_calc_en ? ml_obj::objs.empty() : true;
			if ( mark_over && diff_over)
			{
				if (!mts_master.try_lock())
				{
...
#ifdef RUN_TIME_CALC
					time = (((double)getTickCount() - time) / getTickFrequency()) * 1000;
					cout << endl << "time of once all : " << time / ml_obj::diff_set.size() << " ms. " << "  fs : " << d_frame / ml_obj::diff_set.size() << " objs : " << ml_obj::diff_set.size() << endl;
#endif
					if (diff_calc_en)
					{
#ifdef RES_OUT
...
#endif
						ml_obj::diff_set.clear();
					}
					
					if (laser_mark_en)//显示标刻物体Mark坐标点数
					{
#ifdef RES_OUT
						for (auto & ow_obj : ml_obj::Ow_objs)
						{
							cout << "obj[" << ow_obj.first << "] : " << endl;
							cout << ow_obj.second << endl;

							Point2f v1 = ow_obj.second[1] - ow_obj.second[0];
							Point2f v2 = ow_obj.second[2] - ow_obj.second[0];
							double  ang = acos(v1.dot(v2) / (norm(v1) * norm(v2))) * 180 / 3.1415926;
							cout << "tripple mark angle : " << ang << endl;
						}
#endif
						ml_obj::Ow_objs.clear();
						cout << endl;
					}
...
int main(int argc, char** argv)
{
	...
	else if (argc == 2)
	{
	  ...
	  calibra2(vec_mat,type,board_num ,real_size);
	   ...
	   string mark_name = string("../../data/mark") + ".jpg";
       obj_scale_detect(img_name, temp_name , atof(scale.c_str()) , vec_mat, false ,false , mark_name);
}
...
time = (double)getTickCount();
		...
		vector<Point2f> src_tri;
		src_tri = matching_template_accelerateds(img, temp);
		CHECK_EXP_RETURN(src_tri.size() != 3, MATCHING_TRIPPLE_FAILED);
...
*/
/*void match_features(Mat_<double> img_1, Mat_<double> img_2)
{
	//Ptr<Feature2D> sift = xfeatures2f::SIFT::create();
	Ptr<ORB> orb = ORB::create(100);
	orb->setFastThreshold(0);

	vector<KeyPoint> keypoints_1, keypoints_2;
	Mat_<double> descriptors_1, descriptors_2;

	orb->detectAndCompute(img_1, Mat_<double>(), keypoints_1, descriptors_1);
	orb->detectAndCompute(img_2, Mat_<double>(), keypoints_2, descriptors_2);

	Mat_<double> ShowKeypoints1, ShowKeypoints2;
	...
	xfeatures2f::matchGMS(img_1.size(), img_2.size(), keypoints_1, keypoints_2, matchesAll, matchesGMS);
	cout << "matchesGMS: " << matchesGMS.size() << endl;

	Mat_<double> finalMatches;
	drawMatches(img_1, keypoints_1, img_2, keypoints_2, matchesGMS, finalMatches, Scalar::all(-1), Scalar::all(-1), vector<char>(), DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);
